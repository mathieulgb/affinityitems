		...
		<div id="{$reco.theme.backgroundContentId}" class="{$reco.theme.backgroundContentClass}">
			...
			{foreach from=$reco.aeproducts item=product name=affinityItemsProducts}
				...
				<li {if $reco.theme.productDisplayOptions} style="height: {$reco.theme.productHeight}px; width: {$reco.theme.productWidth}px;{if !$reco.theme.productBackgroundColorTransparent} background-color: {$reco.theme.productBackgroundColor}{/if}; border: {$reco.theme.productBorderSize}px solid {$reco.theme.productBorderColor}; border-radius: {$reco.theme.productBorderRoundedSize}px; {if $smarty.foreach.affinityItemsProducts.iteration%$nbItemsPerLine != 0} margin-right: {$reco.theme.productMarginRight}px{/if}" {/if} id="{$reco.theme.productId}" class="{$reco.theme.productClass} {if $smarty.foreach.affinityItemsProducts.first}first_item{elseif $smarty.foreach.affinityItemsProducts.last}last_item{else}item{/if} {if $smarty.foreach.affinityItemsProducts.iteration%$nbItemsPerLine == 0}last_item_of_line{elseif $smarty.foreach.affinityItemsProducts.iteration%$nbItemsPerLine == 1} {/if} {if $smarty.foreach.affinityItemsProducts.iteration > ($smarty.foreach.affinityItemsProducts.total - $totModulo)}last_line{/if}">
					<a href="{$product.link|escape:'html'}" title="{$product.name|escape:html:'UTF-8'}" id="{$reco.theme.pictureLinkId}" class="{$reco.theme.pictureLinkClass}">
					...
					{if $reco.theme.productTitleActivation} <p {if $reco.theme.productTitleDisplayOptions} style="height: {$reco.theme.productTitleHeight}px" {/if} class="{$reco.theme.productTitleClass}" id="{$reco.theme.productTitleId}"><a {if $reco.theme.productTitleDisplayOptions} style="color:{$reco.theme.productTitleColor};font-size: {$reco.theme.productTitleSize}px;text-align: {$reco.theme.productTitleAlign}; line-height: {$reco.theme.productTitleLineHeight}px;"{/if} href="{$product.link|escape:'html'}" title="{$product.name|truncate:50:'...'|escape:'htmlall':'UTF-8'}">{$product.name|truncate:35:'...'|escape:'htmlall':'UTF-8'}</a></p>{/if} {if $reco.theme.productDescriptionActivation}
					<div {if $reco.theme.productDescriptionDisplayOptions} style="height: {$reco.theme.productDescriptionHeight}px" {/if} id="{$reco.theme.productDescriptionId}" class="{$reco.theme.productDescriptionClass}">
						<a {if $reco.theme.productDescriptionDisplayOptions} style="color:{$reco.theme.productDescriptionColor};font-size: {$reco.theme.productDescriptionSize}px; text-align: {$reco.theme.productDescriptionAlign}; line-height: {$reco.theme.productDescriptionLineHeight}px;"{/if} href="{$product.link|escape:'html'}" title="{l s='More' mod='homefeatured'}">{$product.description_short|strip_tags|truncate:65:'...'}</a></div>{/if}
					<div>
						{if $reco.theme.detailActivation}<a {if $reco.theme.detailDisplayOptions} style="color: {$reco.theme.detailColor}; font-size: {$reco.theme.detailSize}px;" {/if} id="{$reco.theme.detailId}" class="{$reco.theme.detailClass}" href="{$product.link|escape:'html'}" title="{l s='View' mod='homefeatured'}">{l s='View' mod='homefeatured'}</a>{/if}
